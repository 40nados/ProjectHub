...
routes.get("/message/:chatId", async (req, res) => {
  ...
  const result = await message_controller.getMessages(req.params.chatId, limit, offset);
...
routes.post("/message/:chatId", async (req, res) => {
  if(req.body.type == 'text'){
    const result = await message_controller.createMessage(req.params.chatId, req.body);
    if (result.error) res.status(result.status).json(result.error);
    else res.send(result);
  }
...
routes.put("/message/:messageId", async (req, res) => {
  const result = await message_controller.editMessage(req.params.messageId, req.body);
  if (result.error) res.status(result.status).json(result.error);
  else res.send(result);
})
