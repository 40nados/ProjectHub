...
const UpdatePublication = async (req, res) => {
    ...
    if (req.file) {
        ...
        try {
            ...
            return res.status(500).json({ error: 'Error deleting old file from S3.' });
        }
    ...
    try {
        ...
        return res.status(200).json({ publication, message: 'Publication updated successfully!' });
    } catch (error) {
        return res.status(500).json({ error: 'Error updating the publication.' });
    }
};
